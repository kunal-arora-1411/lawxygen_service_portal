...
This documents every route currently implemented in the backend, with exact paths, payloads, and response shapes, for frontend integration. It supersedes `BACKEND_API_SPECS.md` as the source of truth for what's actually live.
...
| GET | `/` | `search?, page?, limit?` | `data: { items, total, page, limit }`. `search` matches name/email/phone (case-insensitive). |
| GET | `/:id` | — | `data`: full user profile (minus password/tokens) + `matters` (last 10) + `matterCount`. |
